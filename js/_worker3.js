...
const PORTS = [443, 80];
const PROTOCOLS = [reverse("sselv"), reverse("najort"), reverse("ss")];
...
const PROXY_HEALTH_CHECK_API = "https://id1.foolvpn.web.id/api/v1/check";
...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    // Build pagination
    document.addPageButton("Prev", `/aio/${page > 0 ? page - 1 : 0}`, page > 0 ? false : true);
    document.addPageButton("Next", `/aio/${page + 1}`, page < Math.floor(proxyList.length / 10) ? false : true);
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/aio")) {
        const page = url.pathname.match(/^\/aio\/(\d+)$/);
        ...
        if (apiPath.startsWith("/domains")) {
        ...
        } else if (apiPath.startsWith("/aio")) {
          const host = request.headers.get("Host");
          const filterCC = url.searchParams.get("cc")?.split(",") || [];
          const filterPort = url.searchParams.get("port")?.split(",") || PORTS;
          const filterVPN = url.searchParams.get("vpn")?.split(",") || PROTOCOLS;
          const filterLimit = parseInt(url.searchParams.get("jumlah")) || 10;
          const filterFormat = url.searchParams.get("format") || "raw";
          const filterDomain = url.searchParams.get("bug") || host;
          const wc = url.searchParams.get("wc");
          let hostVal = host;
          let sniVal = host;

          if (wc === "true" && filterDomain) {
            hostVal = `${filterDomain}.${host}`;
            sniVal = `${filterDomain}.${host}`;
          }
          ...
          for (const proxy of proxyList) {
            const uri = new URL(`${reverse("najort")}://${filterDomain}`);
            uri.searchParams.set("encryption", "none");
            uri.searchParams.set("type", "ws");
            uri.searchParams.set("host", hostVal);
            ...
            for (const protocol of filterVPN) {
              for (const port of filterPort) {
                ...
                uri.searchParams.set("security", port == 443 ? "tls" : "none");
                port == 80 ? uri.searchParams.delete("sni") : uri.searchParams.set("sni", sniVal);
                //uri.searchParams.set("sni", port == 80 && protocol == reverse("sselv") ? "" : sniVal);
                uri.searchParams.set("path", `/${proxy.proxyIP}-${proxy.proxyPort}`);

                uri.hash = `${getFlagEmoji(proxy.country)} ${port == 443 ? "TLS" : "NTLS"} - ${proxy.org}`;
...
<html lang="en" id="html" class="scroll-auto scrollbar-hide dark">
  ...
  <body class="bg-primary-dark font-sans text-text-light bg-fixed relative">
    ...
    <div class="ml-16 flex flex-col items-center min-h-screen relative z-10 p-4">
      <div
        class="rounded-xl p-4 text-center w-full mb-6 shadow-lg glass-effect"
      >
        <div class="flex justify-center gap-3 text-sm flex-wrap text-gray-300">
          <p>
            <span class="text-blue-400 font-semibold">IP:</span>
            <span id="container-info-ip">127.0.0.1</span>
          </p>
          <p>
            <span class="text-green-400 font-semibold">Country:</span>
            <span id="container-info-country">Singapore</span>
          </p>
          <p>
            <span class="text-yellow-400 font-semibold">ISP:</span>
            <span id="container-info-isp">Localhost</span>
          </p>
        </div>
      </div>
...
2
    ...
    <script>
      ...
      function checkProxy() {
        for (let i = 0; ; i++) {
          ...
          new Promise(async (resolve) => {
            const res = await fetch("https://" + host + "/check?target=" + target)
              .then(async (res) => {
                if (isActive) return;
                if (res.status == 200) {
      ...
      function checkGeoip() {
        ...
        const res = fetch("https://" + host + "/api/v1/myip").then(async (res) => {
          if (res.status == 200) {
            const respJson = await res.json();
            containerIP.innerText = respJson.ip;
            containerCountry.innerText = respJson.country;
            containerISP.innerText = respJson.asOrganization;
          }
        });
      }
...
class Document {
  ...
  buildCountryFlag() {
    ...
    for (const flag of new Set(flagList)) {
      flagElement += `<a href="/aio?cc=${flag}${
  ...
  build() {
    ...
    this.html = this.html.replaceAll("PLACEHOLDER_HOSTNAME", this.url.hostname);
