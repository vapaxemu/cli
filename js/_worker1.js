    ...
    async element(element) {
      ...
      if (href) {
        ...
        if (cssResponse.ok) {
          ...
          element.replace(`<style>${cssText}</style>`, {
            html: true,
          });
        }
      }
    ...
    try {
      ...
      if (upgradeHeader === "websocket") {
        ...
        if (proxyListMatch) {
          ...
          console.log(
            `Selected Proxy (/PROXYLIST/${countryCodes.join(",")}): ${prxIP}`,
          );
        ...
        if (allMatch) {
          ...
          if (proxies.length === 0) {
            ...
            if (index === null) {
              ...
              if (proxiesByIndex.length === 0) {
                return new Response(
                  `No proxy at index ${index + 1} for any country`,
                  { status: 404 },
                );
              }
              ...
              selectedProxy =
                proxiesByIndex[
                  Math.floor(Math.random() * proxiesByIndex.length)
                ];
            }
          ...
          console.log(
            `Selected Proxy (/ALL${index !== null ? index + 1 : ""}): ${prxIP}`,
          );
          return await websocketHandler(request);
        }
        ...
        if (putarMatch) {
          const countryCount = putarMatch[1]
            ? parseInt(putarMatch[1], 10)
            : null;
          const proxies = await getPrxList(env.PRX_BANK_URL);
          ...
          if (proxies.length === 0) {
            ...
            if (countries.length === 0) {
              return new Response(`No proxies available`, {
                status: 404,
              });
            }
            ...
            const randomProxy =
              selectedProxies[
                Math.floor(Math.random() * selectedProxies.length)
              ];
            prxIP = `${randomProxy.prxIP}:${randomProxy.prxPort}`;
          }
        ...
        if (regionMatch) {
          ...
          const index = regionMatch[2]
            ? parseInt(regionMatch[2], 10) - 1
            : null;
          ...
          if (REGION_MAP[regionKey] !== undefined) {
            ...
            if (proxies.length === 0) {
              ...
              if (availableProxies.length === 0) {
                return new Response(
                  `No proxies available for region: ${regionKey}`,
                  { status: 404 },
                );
              }
            ...
            console.log(
              `Selected Proxy (/${regionKey}${index !== null ? index + 1 : ""}): ${prxIP}`,
            );
            return await websocketHandler(request);
          }
        }
        ...
        if (countryMatch) {
          ...
          if (proxies.length === 0) {
            ...
            if (!kvPrx[countryCode] || kvPrx[countryCode].length === 0) {
              return new Response(
                `No proxies available for country: ${countryCode}`,
                { status: 404 },
              );
            }
          ...
          console.log(
            `Selected Proxy (/${countryCode}${index !== null ? index + 1 : ""}): ${prxIP}`,
          );
          return await websocketHandler(request);
        }
        ...
        if (url.pathname.length === 3 || url.pathname.includes(",")) {
          ...
          if (kvPrx[prxKey] && kvPrx[prxKey].length > 0) {
            ...
            return new Response(`No proxies available for country: ${prxKey}`, {
              status: 404,
            });
          }
        }
